...
for prod, cons, msg_size in itertools.product(producers, consumers,
message_sizes):
    if (msg_size == 1024):
        run_producer_test(prod, msg_size, num_records[0], throughput[0])
        run_consumer_test(cons, num_records[0])
    else:
        run_producer_test(prod, msg_size, num_records[1], throughput[1])
        run_consumer_test(cons, num_records[1])
